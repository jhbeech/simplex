...
class LpProblem:
    ...
    def primal_simplex(
        ...
        for it in range(max_iterations):
            ## A_b is in the order of basis_vars (does not need to be a particular order)
            A_b = A[:, basis_vars]
            ...
            if log_iterations:
                print(
                    f"\riterations: {it}. Objective: {get_objective_value(c_b, A_b_inv, b)}",
                    end="",
                )
    ...
    def dual_simplex(
        ...
        for it in range(max_iterations):
            ...
            ## need to replace leaving var with entering var
            ## (ie make sure they are in same loc) for sherman morrison algorithm to work
            entering_var = non_basis_vars[entering_var_loc]
            basis_vars[leaving_var_loc] = entering_var
            non_basis_vars[entering_var_loc] = leaving_var
        self.basis = {
            var: val
            for var, val in zip(basis_vars, get_basis_values(A_b_inv, b))
        }
        self.obj = get_objective_value(c_b, A_b_inv, b)
        return self.obj
    ...
    def add_gomory_cut_constraint(self) -> None:
        ...
        frac_var_loc = basis_vars.index(frac_var)
        A_b = A[:, basis_vars]
        ## maybe let's save A_b to self
        ## at the end of simplex instead
        ## so we don't need to do the above

        A_b_inv = np.linalg.inv(A_b)
        p = A_b_inv[frac_var] @ b
        Q = A_b_inv[frac_var, :] @ A[:, non_basis_vars]

        ## new constraint is:
        ## x_i ≤ floor(p[i]) + ceil(Q[i,:]) @ x_N
        ## where i is the loc of the frac variable
        ## in the ordered list of basis vars

        ## x_i - floor(p[i]) + ceil(Q[i,:]) @ x_N ≤ floor(p[i])
        ## this needs to be reordered to correspond with the
        ## ordering of the variables

        ## ([x_0,x_1, .., x_N] @ ∂(i)) + ceil(Q[i,:]) @ [x_NB_1, ... X_NB_k]
        ## ≤ floor(p[i])
...
def branch_and_bound(
    ...
    if queue is None:
        ...
        return (lower_bound, upper_bound, [(lower_bound, upper_bound)], 0, 0)
...
def get_reduced_costs(
    ...
    """returns reduced costs in order of non basis variables (order determining A_n, c_n).
...
def get_inv_sherman_morrison(
    ...
    ## every 50 iterations explicitly calc
    ## the inverse for stability
...
def check_simplex(n, m, max_val):
    ...
    prob = LpProblem(A, b, c, slacks_included=True)
    prob.set_initial_basis()
    prob.primal_simplex(1000)
    basis = prob.basis
    x_simplex_solver = [basis.get(i, 0) for i in range(A.shape[1])]

    prob_pulp = pulp.LpProblem("pulp", pulp.LpMaximize)
    x_pulp = pulp.LpVariable.matrix(
        "x", range(A.shape[1]), 0, None, pulp.LpContinuous
    )
    prob_pulp += c @ x_pulp
    for lhs, rhs in zip(A @ x_pulp, b):
        prob_pulp += lhs == rhs
    status = prob_pulp.solve(pulp.PULP_CBC_CMD(msg=0))
    print(prob_pulp.objective.value())
    for i, j in zip(x_simplex_solver, x_pulp):
        print(i, j.value())


if __name__ == "__main__":
    np.random.seed(42)
    check_simplex(12, 12, 100)
    # np.random.seed(42)
    # n = 5
    # m = 5
    # max_val = 10
